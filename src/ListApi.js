...
export default function ListApi() {
 ...
 //    return 'is Loading...';
  ...
  async function changeAttendanceStatus(id, value) {
    ...
    fetchGuests().catch(() => console.log('changing attendance status went wrong'),);
  }
          ...
          {guest.map((guestList) => {
            return (
              <div key={guestList.id}>
                    ...
                    <label>
                      <input aria-label={`attending status ${guestList.firstName} ${guestList.lastName}`}
                      type="checkbox"
                    ...
                    <button aria-label={`Remove ${guestList.firstName} ${guestList.lastName}`}
                    onClick={() => {
